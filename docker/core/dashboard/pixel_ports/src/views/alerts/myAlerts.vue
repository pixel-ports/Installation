<template>
  <div class="app-container" :style="bgc">
    ...
    >
      <el-table-column :label="$t('alert.alertFecha')" min-width="180px" sortable>
      ...
      <el-table-column :label="$t('alert.alertTitle')" min-width="50px" prop="from" sortable />
      <el-table-column :label="$t('alert.alertDescription')" min-width="100px" prop="text" sortable />
      <!-- <el-table-column :label="$t('alert.alertType')" min-width="50px" property="notification.type" /> -->
    </el-table>
...
export default {
  ...
  computed: {
    filteredBySearchText() {
      return this.myAlerts.filter(alert => alert.from.toLowerCase().includes(this.searchText.toLowerCase()) || alert.text.toLowerCase().includes(this.searchText.toLowerCase()))
    },
